...
impl VerboseDisplay for StakePoolOutput {
    fn write_str(&self, w: &mut dyn Write) -> Result {
        ...
        writeln_name_value(
            w,
            "  Pool stake address:",
            &find_pool_stake_address(&reflect_single_pool::id(), &self.pool_address).to_string(),
        )?;
        writeln_name_value(
            w,
            "  Pool mint address:",
            &find_pool_mint_address(&reflect_single_pool::id(), &self.pool_address).to_string(),
        )?;
        writeln_name_value(
            w,
            "  Pool stake authority address:",
            &find_pool_stake_authority_address(&reflect_single_pool::id(), &self.pool_address)
                .to_string(),
        )?;
        writeln_name_value(
            w,
            "  Pool mint authority address:",
            &find_pool_mint_authority_address(&reflect_single_pool::id(), &self.pool_address)
                .to_string(),
        )?;
        writeln_name_value(
            w,
            "  Pool MPL authority address:",
            &find_pool_mpl_authority_address(&reflect_single_pool::id(), &self.pool_address)
                .to_string(),
        )?;
