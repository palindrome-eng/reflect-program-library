...
use {
    crate::{
        ...
        instruction::ReflectPoolInstruction,
        state::{ReflectPool, ReflectPoolAccountType},
...
fn get_stake_state(stake_account_info: &AccountInfo) -> Result<(Meta, Stake), ProgramError> {
    ...
    match stake_state {
        ...
        _ => Err(ReflectPoolError::WrongStakeStake.into()),
    }
}
...
fn check_vote_account(vote_account_info: &AccountInfo) -> Result<(), ProgramError> {
    ...
    let state_variant = vote_account_data
        ...
        .ok_or(ReflectPoolError::UnparseableVoteAccount)?;

    match u32::from_le_bytes(state_variant) {
        1 | 2 => Ok(()),
        0 => Err(ReflectPoolError::LegacyVoteAccount.into()),
        _ => Err(ReflectPoolError::UnparseableVoteAccount.into()),
    }
}
...
fn check_mpl_metadata_account_address(
    ...
    if metadata_account_pubkey != *metadata_address {
        Err(ReflectPoolError::InvalidMetadataAccount.into())
    } else {
        Ok(())
    }
}
...
impl Processor {
    ...
    fn process_initialize_pool(program_id: &Pubkey, accounts: &[AccountInfo]) -> ProgramResult {
        ...
        let pool_space = get_packed_len::<ReflectPool>();
        if !rent.is_exempt(pool_info.lamports(), pool_space) {
            return Err(ReflectPoolError::WrongRentAmount.into());
        }
        if pool_info.data_len() != 0 {
            return Err(ReflectPoolError::PoolAlreadyInitialized.into());
        }
        ...
        let mut pool = try_from_slice_unchecked::<ReflectPool>(&pool_info.data.borrow())?;
        pool.account_type = ReflectPoolAccountType::Pool;
    ...
    fn process_reactivate_pool_stake(
        ...
        ReflectPool::from_account_info(pool_info, program_id)?;
        ...
        if pool_stake_state.delegation.deactivation_epoch > clock.epoch {
            return Err(ReflectPoolError::WrongStakeStake.into());
        }
    ...
    fn process_deposit_stake(program_id: &Pubkey, accounts: &[AccountInfo]) -> ProgramResult {
        ...
        if pool_stake_info.key == user_stake_info.key {
            return Err(ReflectPoolError::InvalidPoolStakeAccountUsage.into());
        }
        ...
        let stake_added = post_pool_stake
            ...
            .ok_or(ReflectPoolError::ArithmeticOverflow)?;
        ...
        if user_stake_info.lamports() != 0 {
            return Err(ReflectPoolError::UnexpectedMathError.into());
        }
        ...
        let new_pool_tokens = calculate_deposit_amount(token_supply, pre_pool_stake, stake_added)
            .ok_or(ReflectPoolError::UnexpectedMathError)?;

        if new_pool_tokens == 0 {
            return Err(ReflectPoolError::DepositTooSmall.into());
        }
    ...
    fn process_withdraw_stake(
        ...
        if withdraw_stake == 0 {
            return Err(ReflectPoolError::WithdrawalTooSmall.into());
        }
        ...
        if withdraw_stake > pre_pool_stake || withdraw_stake == pool_stake_info.lamports() {
            return Err(ReflectPoolError::WithdrawalTooLarge.into());
        }
    ...
    fn process_create_pool_token_metadata(
        ...
        let inline_mpl_token_metadata_program_info = next_account_info(account_info_iter)?;
        let system_program_info = next_account_info(account_info_iter)?;

        let pool = ReflectPool::from_account_info(pool_info, program_id)?;
        ...
        check_mpl_metadata_program(inline_mpl_token_metadata_program_info.key)?;
        check_mpl_metadata_account_address(metadata_info.key, pool_mint_info.key)?;

        if !payer_info.is_signer {
            msg!("Payer did not sign metadata creation");
            return Err(ReflectPoolError::SignatureMissing.into());
        }
        ...
        let new_metadata_instruction = create_metadata_accounts_v3(
            *inline_mpl_token_metadata_program_info.key,
            *metadata_info.key,
            *pool_mint_info.key,
            *pool_mint_authority_info.key,
            *payer_info.key,
    ...
    fn process_update_pool_token_metadata(
        ...
        if *authorized_withdrawer_info.key != vote_account_withdrawer {
            ...
            return Err(ReflectPoolError::InvalidMetadataSigner.into());
        }
    ...
    pub fn process(program_id: &Pubkey, accounts: &[AccountInfo], input: &[u8]) -> ProgramResult {
        ...
        match instruction {
            ...
            ReflectPoolInstruction::CreateTokenMetadata => {
                msg!("Instruction: CreateTokenMetadata");
                Self::process_create_pool_token_metadata(program_id, accounts)
            }
            ReflectPoolInstruction::UpdateTokenMetadata { name, symbol, uri } => {
