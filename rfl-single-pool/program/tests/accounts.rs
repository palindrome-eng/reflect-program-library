...
use {
    ...
    reflect_single_pool::{
        ...
        id,
        instruction::{self, ReflectPoolInstruction},
    },
    test_case::test_case,
};
...
async fn fail_account_checks(test_mode: TestMode) {
    ...
    for j in 0..instructions[i].accounts.len() {
        ...
        if prev_pubkey == accounts.pool {
            check_error(e, ReflectPoolError::InvalidPoolAccount)
        } else if prev_pubkey == accounts.stake_account {
            check_error(e, ReflectPoolError::InvalidPoolStakeAccount)
        } else if prev_pubkey == accounts.stake_authority {
            check_error(e, ReflectPoolError::InvalidPoolStakeAuthority)
        } else if prev_pubkey == accounts.mint_authority {
            check_error(e, ReflectPoolError::InvalidPoolMintAuthority)
        } else if prev_pubkey == accounts.mpl_authority {
            check_error(e, ReflectPoolError::InvalidPoolMplAuthority)
        } else if prev_pubkey == accounts.mint {
            check_error(e, ReflectPoolError::InvalidPoolMint)
...
fn make_basic_instruction(
    accounts: &ReflectPoolAccounts,
    instruction_type: ReflectPoolInstruction,
) -> Instruction {
    match instruction_type {
        ...
        ReflectPoolInstruction::DepositStake => instruction::deposit_stake(
            &id(),
            &accounts.pool,
            &Pubkey::default(),
            &Pubkey::default(),
        ...
        ReflectPoolInstruction::WithdrawStake { .. } => instruction::withdraw_stake(
            &id(),
            &accounts.pool,
            &Pubkey::default(),
            &Pubkey::default(),
        ...
        ReflectPoolInstruction::UpdateTokenMetadata { .. } => instruction::update_token_metadata(
            &id(),
            &accounts.vote_account.pubkey(),
            &accounts.withdrawer.pubkey(),
            "".to_string(),
...
fn consistent_account_order() {
    ...
    let instructions = vec![
        make_basic_instruction(&accounts, ReflectPoolInstruction::InitializePool),
        make_basic_instruction(&accounts, ReflectPoolInstruction::ReactivatePoolStake),
        make_basic_instruction(&accounts, ReflectPoolInstruction::DepositStake),
        ...
        make_basic_instruction(&accounts, ReflectPoolInstruction::CreateTokenMetadata),
        make_basic_instruction(
            &accounts,
            ReflectPoolInstruction::UpdateTokenMetadata {
                name: "".to_string(),
                symbol: "".to_string(),
                uri: "".to_string(),
            },
