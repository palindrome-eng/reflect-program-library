...
use {
    ...
    reflect_single_pool::{
        error::ReflectPoolError, find_default_deposit_account_address, id, instruction,
    },
    test_case::test_case,
};
...
async fn fail_bad_account(activate: bool, automorph: bool) {
    ...
    if automorph {
        check_error(e, ReflectPoolError::InvalidPoolStakeAccountUsage);
    } else {
        check_error(e, ReflectPoolError::WrongStakeStake);
    }
}
